...
class BatchShipNotifyHandlerTM(TMBaseReqHandler):
    @coroutine
    def post(self):
        """
        处理开始批量发货请求
        ...
        try:
            ...
            query = self.db.query(TShipOrder).filter(TShipOrder.ship_id == self.msg_info[MSG_TASK_NO],
                                                     TShipOrder.type == OrderSyncDBType.SHIP_ORDER.value)
            ...
            query.update({
                TShipOrder.status: ShipOrderState.SHIPPING_STATE.value
            })
            ...
            self.register_ship_count_transaction(msg_info[MSG_TASK_NO], EpcSyncType.SHIP.value)

        except Exception as err_info:
            self.error("handle batch ship start failed: %s", err_info)
            self.set_status(500)
            self.write({MSG_STATUS: ResponseStatus.SYSTEM_ERROR.value,
                        MSG_STATUS_TEXT: response_desc[ResponseStatus.SYSTEM_ERROR]})
            self.finish()
